...
public class DepthFirstPathsAllNodes
{
    ...
    public DepthFirstPathsAllNodes(int[][] graph)
    {
        _marked = new bool[graph.Length];
    }

    public List<int> Dfs(int[][] graph)
    {
    ...
    private (List<int> Path, List<int> Return) Dfs(int[][] graph, int v, int? from)
    {
        ...
        {
            returnPath.AddRange(childrenToMerge[childrenToMerge.Length - 1].Return);
            returnPath.Add(v);
        }
